...
class Model(nn.Module):
    ...
    def __init__(self, configs, version='fourier', mode_select='random', modes=32):
        ...
        if self.task_name == 'classification_cross_domain':
            ...
            self.dropout = nn.Dropout(configs.dropout)
            self.projection = nn.Linear(configs.d_model * configs.seq_len, configs.num_class)
            self.domain_classifier = DomainClassifier(configs.d_model * configs.seq_len, configs.domain_hidden, 1)
    ...
    def classification_cross_domain(self, x_enc, x_mark_enc):
        # enc
        enc_out = self.enc_embedding(x_enc, None)
        enc_out, attns = self.encoder(enc_out, attn_mask=None)

        # x_emb作为情绪分类器和领域分类器的输入
        x_emb = self.act(enc_out)
        x_emb = self.dropout(x_emb)
        x_emb = x_emb * x_mark_enc.unsqueeze(-1)
        x_emb = x_emb.reshape(x_emb.shape[0], -1)

        # emotion classification
        class_logit = self.projection(x_emb)

        # Domain classification
        domain_logit = self.domain_classifier(x_emb)
        domain_logit = torch.sigmoid(domain_logit)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.task_name == 'classification':
            ...
            return dec_out  # [B, N]
        if self.task_name == 'classification_cross_domain':
            dec_out, domain_output = self.classification_cross_domain(x_enc, x_mark_enc)
            return dec_out, domain_output
        return None
