...
class Predictor(nn.Module):
    ...
    def __init__(self, dim, num_types):
        ...
        # nn.init.xavier_normal_(self.linear.weight) 会对该层的权重参数进行 Xavier 初始化，使得其具有较好的数值范围。
...
class Decoder(nn.Module):
    ...
    def forward(self, x_dec, x_mark_dec, refer):
        ...
        # 第一个decoder layer的输出作为第二个decoder layer的输入
        dec_enc, _ = self.layers[0](dec_enc, refer, refer)

        # 将reference和第一个decoder layer的输出进行拼接
        refer_enc = torch.cat([refer, dec_enc], dim=1)

        # 将mask重复len(dec_enc)次，然后转移到dec_enc所在的device上
